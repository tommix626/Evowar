...
class CLS_GameAgent(object):
    ...
    def pilot_eat_dot_check(self):
        for dot in self.dotList:
            ...
            distance = (dotpos[0] - self.pilot.pos[0]) ** 2 + (
                dotpos[1] - self.pilot.pos[1]
            ) ** 2
            if distance < dotplayerbound:
                ...
                self.displaytips(
                    "exp+", dot.pts
                )  # FIXME a word box pop out to remind the player
        return

    def draw_fence(self):
        x, y = (
            self.pilot.fpos[0] - self.pilot.pos[0],
            self.pilot.fpos[1] - self.pilot.pos[1],
        )
    ...
    def build_item(
        ...
        if itemtag == "shooter":
            ...
            itemtag == "factory"
        ):  # create troops every several hundreds frame(various kinds)
            pass
        elif (
            itemtag == "hptower"
        ):  # shoot atk<0 bullet(+HP) or have a range healing affect
            pass
            ...
            itemtag == "poisoner_T"
        ):  # with poison around it self,kill all who goes into it(atk should be very low)
            self.pilot.exp = self.pilot.exp - expConsumption[2]
            if dtype == "range":
                tpos = [
                    self.pilot.pos[0] + random.randint(-20, 20),
            ...
            itemtag == "add_on_T"
        ):  # add on troops and buildings to give them buffs(or debuffs on enemy)
            pass
        elif itemtag == "rand_add_on":  # random(buff/debuff) very low cost
            pass
        elif (
            itemtag == "suiside_team"
        ):  # high speed small range suiside troop_team(3 in a roll)
            pass
        elif itemtag == "exp_gainer":  # gives you a constant way to increase Exp
            pass
        elif (
            itemtag == "map_destroyer"
        ):  # with unlimited bullet range and bullet dur(long interval)
            pass
        elif (
            itemtag == "mine"
        ):  # explosive(building,that are in special mineList/fake_pos)
            pass
        elif (
            itemtag == "portal"
        ):  # experimental player-use one-way(2ways also ok) portal(at most one pair on map first click drops ins second drops outs)
            pass
        elif itemtag == "dummy":  # a robot without atk but can attract fire(HP++)
            pass
        elif (
            itemtag == "healer"
        ):  # shoot bullet to heal other troop-members(player excluded)
            pass
        elif (
            itemtag == "line-atk_T"
        ):  # draw lines between tower and enemy,cause an increasing atk at each one troop
            pass
        elif (
            itemtag == "alert_tower"
        ):  # contains a certain amount of troops,release all when detect enemy
            pass
        elif (
            itemtag == "patrol_team"
        ):  # several troops patrolling on a line (two clicks to place)
            pass
