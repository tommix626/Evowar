...
class Renderer:
    ...
    def draw_minimap_background(self,x_start_location,x_len,y_len):
        pygame.draw.rect(self.surface,(255,255,255),(x_start_location-3,0,x_len+2,y_len+2))
        pygame.draw.rect(self.surface,(0,0,0),(x_start_location-2,1,x_len,y_len))
    ...
    def draw_cls(self,cls,fpos,ppos,tag=0):#use player Position to draw self
        ...
        if(dx+cls.rad<0 or dx-cls.rad>SCREEN_W or dy+cls.rad<0 or dy-cls.rad>SCREEN_H):
            if(tag==1):
                print("out of range")
            return -1
        if(tag==0):
